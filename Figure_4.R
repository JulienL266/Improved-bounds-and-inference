...
#Plots for ICU example with Stensrud et al. results
width = 3
par(mar = c(5,5,1,1), cex.axis = 1.5, cex.lab = 1.5)
plot(NULL, ylim = c(0,1), xlim = c(0.25,1.7), 
     xlab = "", ylab = expression("E(Y"^"g"*")"), xaxt = 'n')
axis(1, at = c(0.3, 0.4, 0.55, 0.65, 0.8, 0.9, 1.05, 1.15, 1.3, 1.4, 1.55, 1.65),
     labels = c(expression("g"["opt"]), expression("g"["sup"]), expression("g"["opt"]), expression("g"["sup"]), expression("g"["opt"]), expression("g"["sup"]), expression("g"["A-opt"]), expression("g"["A-sup"]), expression("g"["opt"]), expression("g"["sup"]), expression("g"["opt"]), expression("g"["sup"])))
axis(1, at = c(0.35, 0.6, 0.85, 1.1, 1.35, 1.6),
     labels = c("Opportunist", "Pessimist", "Healthcare","Conventionality" ,"Optimist", "Stensrud et al."), tick = FALSE, line = 1)

abline(h = 0)
#Observed regime CI
obs_CI <- c(mean(dat$Y) - qt(0.975, df = length(dat$Y)-1)*sd(dat$Y)/sqrt(length(dat$Y)), mean(dat$Y) + qt(0.975, df = length(dat$Y)-1)*sd(dat$Y)/sqrt(length(dat$Y)))
x_sep <- seq(0, 2, by = 0.001)
y.low <- rep(obs_CI[1], length(x_sep))
y.high <- rep(obs_CI[2], length(x_sep))
polygon(x = c(x_sep, rev(x_sep)), y = c(y.low, rev(y.high)), col =  adjustcolor("orangered", alpha.f = 0.10), border = NA)
abline(h = mean(dat$Y), col = "red")
#axis(2, at = c(mean(dat$Y)), labels = c("E(Y)"), col = "red", tick = FALSE, line = 1)
#Opportunist
arrows(x0=0.3, y0=max(0,minimax_CI[1]), x1=0.3, 
       y1=min(1,minimax_CI[2]), 
       code=3, angle=90, length=0.05, lwd=width, col = 'red4')
arrows(x0=0.3, y0=max(0,lower_bound.minimax_opt), x1=0.3, y1=min(1,upper_bound.minimax_opt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'cyan')
arrows(x0=0.4, y0=max(0,minimax_superopt_CI[1]), x1=0.4, 
       y1=min(1,minimax_superopt_CI[2]), 
       code=3, angle=90, length=0.05, lwd=width, col = 'red4')
arrows(x0=0.4, y0=max(0,lower_bound.minimax_superopt), x1=0.4, 
       y1=min(1,upper_bound.minimax_superopt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'cyan')
#Pessimist
arrows(x0=0.55, y0=max(0,maximin_CI[1]), x1=0.55, 
       y1=min(1,maximin_CI[2]), 
       code=3, angle=90, length=0.05, lwd=width, col = 'green')
arrows(x0=0.55, y0=max(0,lower_bound.maximin_opt), x1=0.55, y1=min(1,upper_bound.maximin_opt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'black')
arrows(x0=0.65, y0=max(0,maximin_superopt_CI[1]), x1=0.65, 
       y1=min(1,maximin_superopt_CI[2]), 
       code=3, angle=90, length=0.05, lwd=width, col = 'green')
arrows(x0=0.65, y0=max(0,lower_bound.maximin_superopt), x1=0.65, y1=min(1,upper_bound.maximin_superopt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'black')
#Healthcare
arrows(x0=0.8, y0=max(0,healthcare_CI[1]), x1=0.8, 
       y1=min(1,healthcare_CI[2]), 
...
arrows(x0=0.9, y0=max(0,lower_bound.healthcare_superopt), x1=0.9, 
       y1=min(1,upper_bound.healthcare_superopt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'azure3')
#Healthcare with observed baseline (opt)
arrows(x0=1.05, y0=max(0,healthcare_obs_baseline_CI.opt[1]), x1=1.05, 
       y1=min(1,healthcare_obs_baseline_CI.opt[2]), 
       code=3, angle=90, length=0.05, lwd=width, col = 'darkblue')
arrows(x0=1.05, y0=max(0,lower_bound.healthcare_obs_baseline.opt), x1=1.05, 
       y1=min(1,upper_bound.healthcare_obs_baseline.opt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'yellow')
#Healthcare with observed baseline
arrows(x0=1.15, y0=max(0,healthcare_obs_baseline_CI[1]), x1=1.15, 
       y1=min(1,healthcare_obs_baseline_CI[2]), 
       code=3, angle=90, length=0.05, lwd=width, col = 'darkblue')
arrows(x0=1.15, y0=max(0,lower_bound.healthcare_obs_baseline), x1=1.15, 
       y1=min(1,upper_bound.healthcare_obs_baseline), 
       code=3, angle=90, length=0.025, lwd=width, col = 'yellow')
#Optimist
arrows(x0=1.3, y0=max(0,maximax_CI[1]), x1=1.3, 
       y1=min(1,maximax_CI[2]),
       code=3, angle=90, length=0.05, lwd=width, col = 'red')
arrows(x0=1.3, y0=max(0,lower_bound.maximax_opt), x1=1.3, y1=min(1,upper_bound.maximax_opt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'blue')
arrows(x0=1.4, y0=max(0,maximax_superopt_CI[1]), x1=1.4, 
       y1=min(1,maximax_superopt_CI[2]), 
       code=3, angle=90, length=0.05, lwd=width, col = 'red')
arrows(x0=1.4, y0=max(0,lower_bound.maximax_superopt), x1=1.4, y1=min(1,upper_bound.maximax_superopt), 
       code=3, angle=90, length=0.025, lwd=width, col = 'blue')
#Stensrud et al.
arrows(x0=1.55, y0=0.4, x1=1.55, 
       y1=1, 
       code=3, angle=90, length=0.05, lwd=width, col = 'purple')
arrows(x0=1.55, y0=0.93, x1=1.55, 
       y1=0.931, #0.001 added for visibility
       code=3, angle=90, length=0.025, lwd=width, col = 'orange')

arrows(x0=1.65, y0=0.77, x1=1.65, 
       y1=1, 
       code=3, angle=90, length=0.05, lwd=width, col = 'purple')
arrows(x0=1.65, y0=0.97, x1=1.65, 
       y1=0.971, #0.001 added for visibility
       code=3, angle=90, length=0.025, lwd=width, col = 'orange')
